...
class TestImagePairDataset(Dataset):
    ...
    Args:
        ...
        split: A text file listing the name of images.
    """
    def __init__(self, size, folder_A, folder_B, mask):
    ...
    def __getitem__(self, index: int):
        ...
        if self.folder_mask != None:
            mask_path = f'{self.folder_mask}/{img_name}'
            with open(mask_path, 'rb') as f:
                mask = Image.open(f)
                mask = T.ToTensor()(mask)
            img_A = img_A * mask.int().float()
            img_B = img_B * mask.int().float()
        return img_A, img_B
